...
def appendTextIcon(textobject, icon, scaleM= 1):
    ...
    # Remove existing object with the same name (optional)
...
def textIcon(iconString,tobj,MapObject, merge = False, scaleMo = 1):

    icon = None

    if iconString != "no" and iconString != "":
        icon = appendTextIcon(tobj,iconString, scaleMo)
        if merge == True:
            icon.scale.z = 10
            merge_with_map(MapObject, icon)
            icon.location.z += 0.35
...
def HexagonInnerText(MapObject):
    ...
    #_progress.ProgressOverlay.get().finish()
...
def single_color_mode_mesh(original, map, tolerance = None):
    ...
    #extrude the obj so the remesh doesnt create weird bevels on the top -> prevent overhangs
    ...
    # Solidify on Duplicate to "upscale it" and create the tolerance
    solid = obj.modifiers.new(name="Solidify", type='SOLIDIFY')
    solid.offset = 1.0
    solid.thickness = tolerance/2
    applyModifier(obj, solid)

    # Remesh (Voxel) on Duplicate
    #remesh = obj.modifiers.new(name="Remesh", type='REMESH')
    #remesh.mode = 'VOXEL'
    #remesh.voxel_size = 0.4
    ...
    # Remesh (Voxel) on the Original part so its easier to print by smoothing out the sharp edges
...
def fetch_osm_data(bbox, kind="WATER", max_cache_age_hours=720, return_cache_status=False):
    ...
    if kind == "STREETS":
        cache_key = make_cache_key(bbox, kind + str(mapsize))
...
{lines}
    ...
    OSM_QUERY_BUILDERS = {
        "WATER": lambda s, w, n, e, **_: _simple_query(s, w, n, e, [
            'way["natural"="water"]',
        ...
        "STREETS": lambda s, w, n, e, mapsize=0, **_: _build_streets_query(s, w, n, e, mapsize),
    }

    def _build_streets_query(s, w, n, e, mapsize):
        if mapsize > const.STREETS_MAJOR_ONLY_THRESHOLD:
            filter_str = '["highway"~"^(motorway|trunk)$"]'
        elif mapsize > const.STREETS_PRIMARY_THRESHOLD:
            filter_str = '["highway"~"^(motorway|trunk|primary|secondary)$"]'
        else:
            filter_str = '["highway"]'
    ...
    query = builder(south, west, north, east, mapsize=mapsize)
    ...
    for attempt in range(apiRetries):
        try:
            ...
            if _ov.active:
                _ov.update(message=f"Timed out — retrying… ({attempt + 1}/{apiRetries})")
            time.sleep(5)
        except Exception as e:
        ...
        for k in range(lats):
            for l in range(lons):
                ...
                for i,element in enumerate(data['elements']):
                    ...
                    zTerrainOffset = RaycastPointToMeshZ((x,y,100),map)
                    ...
                    z_offset = height * 0.002 * scaleHor # * base_elevation
                    ...
                    for (x, y, nlat, nlon) in footprint:
                        ...
                        verts.append((xb, yb, base_elevation))
...
def coloring_main(map,kind = "WATER"):
    ...
    if created_objects:
        ...
        for o1 in created_objects_booleaned:
            for o2 in created_negatives_booleaned:
                if is_bbox_overlapping(o1,o2):
                    pass
                    boolean_operation(o1,o2)
                    recalculateNormals(o1)


        remove_objects(created_negatives_booleaned)
...
def createOcean(bboxBigger,waterHeight,scaleHor,landpoints, baseplate, tile, baseHeight):
    ...
    if coastcurve != None:
        ...
        if elementMode == "PAINT":
            ...
            return None
        elif elementMode == "SINGLECOLORMODE":
...
def writeMetadata(obj, type = "MAP"):
    ...
    if type == "MAP":
        ...
        obj["el_sActive"] = bpy.context.scene.tp3d.el_sActive
        ...
        obj["singleColorModeElements"] = bpy.context.scene.tp3d.singleColorModeElements
    ...
    if type == "CITY" or type == "WATER" or type == "FOREST" or type == "GLACIER" or type == "FARMLAND" or type == "SCREE":
        obj["Object type"] = type
        obj["Addon"] = bl_info["name"]
        obj["Version"] = bl_info["version"]
        obj["minThickness"] = bpy.context.scene.tp3d.minThickness
        ...
        obj["ExportGroup"] = 0 if bpy.context.scene.tp3d.elementMode == "SINGLECOLORMODE" else 1
    
    if type == "BUILDINGS" or type == "STREET":
...
def projection(operation, Mapobject, obj):
    ...
    if operation == "singleColorMode":
        ...
        thickerObj = single_color_mode_mesh(obj,Mapobject)
...
def _rg_build_terrain_elements(obj, scaleHor, curveObj=None):
    ...
    # --------------------------------------------------
    COLORING_ELEMENTS = [
        ('water',   'col_wActive',   const.WATER_MAXSIZE,   "Water",   "Fetching water data…"),
    ...
    _active_elem_flags = (
        [flag for _, flag, size, _, _ in COLORING_ELEMENTS if getattr(tp3d, flag) == 1 and map_km <= size]
        + (['_ocean']    if tp3d.el_oActive == 1 else [])
        + (['_buildings'] if tp3d.el_bActive == 1 and map_km <= const.BUILDINGS_MAXSIZE else [])
        + (['_roads']    if tp3d.el_sActive == 1 and map_km <= const.ROADS_MAXSIZE else [])
    )
    ...
    if tp3d.el_oActive == 1:
        ...
        ocean_pad_lon = (maxLon - minLon) * 0.10
        terrain['ocean'] = createOcean(
            (minLat - ocean_pad_lat, minLon - ocean_pad_lon, maxLat + ocean_pad_lat, maxLon + ocean_pad_lon),
...
def _rg_apply_single_color_mode(obj, curveObjs, terrain, props):
    ...
    if props['elementMode'] == "SINGLECOLORMODE":
        ...
        for i, key in enumerate(TERRAIN_PRIORITY_ORDER):
            ...
            thicker = single_color_mode_mesh(elem_obj, obj)
...
def _rg_assign_materials_and_export(obj, curveObjs, textobj, plateobj, props, buggyDataset, start_time, exportformat, terrain=None):
    ...
    if is_3mf_extension_installed() and not getattr(tp3d_props, 'disable_3mf_export', False):
        ...
        if terrain and props.get('elementMode') == "SEPARATE" or props.get('elementMode') == "SINGLECOLORMODE":
            for elem_obj in terrain.values():
                if elem_obj and elem_obj.name in bpy.data.objects:
                    elem_obj.select_set(True)

        # Text object: material + select
        if shape in {"HEXAGON INNER TEXT", "HEXAGON OUTER TEXT", "OCTAGON OUTER TEXT", "HEXAGON FRONT TEXT", "MEDAL"}:
        ...
        if terrain and props.get('elementMode') == "SEPARATE":
            for elem_obj in terrain.values():
                if elem_obj and elem_obj.name in bpy.data.objects:
                    export_to_STL(elem_obj, exportformat)

        # Text object: material + export
        if shape in {"HEXAGON INNER TEXT", "HEXAGON OUTER TEXT", "OCTAGON OUTER TEXT", "HEXAGON FRONT TEXT", "MEDAL"}:
            mat_name = "TRAIL" if shape == "HEXAGON INNER TEXT" else "WHITE"
            mat = bpy.data.materials.get(mat_name)
            textobj.data.materials.clear()
...
def runGeneration(type):
    ...
    overlay.add_completed_step("Map shape created")
    ...
    overlay.add_completed_step("Trail curve built")
    ...
    terrain = _rg_build_terrain_elements(obj, scaleHor, curveObj=curveObjs[0] if curveObjs else None)

    # --- Phase 15: Single color mode processing ---
    overlay.update(0.95, "Coloring", "Applying single-color mode…")
    _rg_apply_single_color_mode(obj, curveObjs, terrain, props)

    return

    # Final road intersection and normal recalculation
    if terrain['roads']:
        boolean_operation(terrain['roads'], MapObject, "INTERSECT")
        terrain['roads'].location.z += 0.4
    recalculateNormals(obj)

    overlay.add_completed_step("Terrain applied")
    ...
    _rg_assign_materials_and_export(
        obj, curveObjs, textobj, plateobj, props, buggyDataset, start_time, exportformat, terrain
    )
    overlay.finish()
